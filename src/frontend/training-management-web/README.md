...
本目录是企业内部培训管理系统的 Vue 3 单页应用。M0～M6公共基础工程已通过用户验收并正式交付；业务页面仍按后续里程碑逐步接入。

## 环境要求

- Node.js `24.18.0`（以 `.nvmrc` 和 `.node-version` 为准）
...
写操作不自动重试；网络中断或超时映射为 `result-unknown`，页面必须引导用户查询最终状态。相同写操作可使用 `SingleFlightController` 共享在途 Promise，列表查询可使用 `LatestRequestController` 取消旧请求。真实认证头、Cookie/Token 与最终响应包裹结构仍等待后端冻结，只能在 API/HTTP 适配层替换。
...
账号与密码仅由 `src/mocks/`中的开发态认证服务加载，不在登录页面展示。Pinia Store 通过`src/services/auth.ts`消费领域化当前用户；非 Mock 构建使用未配置认证适配器，构建产物不得包含上述账号或密码。
