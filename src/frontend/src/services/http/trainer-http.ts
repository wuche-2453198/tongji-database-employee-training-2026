...
const toPayload = (input: TrainerInput) => ({
  TrainerName: input.name,
  Title: input.title || undefined,
  Company: input.company || undefined,
  Phone: input.phone || undefined,
  Email: input.email || undefined,
  StarLevel: input.starLevel,
  IsInternal: input.isInternal ? 'Y' : 'N',
})
export const httpTrainerService: TrainerService = {
  async list(query, options) {
    const response = await requestApi<ApiEnvelopeDto<PageDto<TrainerDto>>>({
      method: 'GET',
      url: '/api/trainers',
      signal: options?.signal,
      operation: 'query',
      params: {
        TrainerName: query.name || undefined,
  ...
  async create(input, options) {
    const response = await requestApi<ApiEnvelopeDto<TrainerDto>>({
      method: 'POST',
      url: '/api/trainers',
      signal: options?.signal,
      operation: 'write',
    ...
    return mapTrainer(response)
  },
  async update(id, input, options) {
    const response = await requestApi<ApiEnvelopeDto<TrainerDto>>({
      method: 'PUT',
      url: `/api/trainers/${encodeURIComponent(id)}`,
      signal: options?.signal,
      operation: 'write',
      data: toPayload(input),
    })
