<script setup lang="ts">
import { computed, onMounted, reactive, ref } from 'vue'
import { useRouter } from 'vue-router'
import AppButton from '@/components/common/AppButton.vue'
import AppPagination from '@/components/common/AppPagination.vue'
import ConfirmDialog from '@/components/common/ConfirmDialog.vue'
import DataTable from '@/components/common/DataTable.vue'
import PageHeader from '@/components/common/PageHeader.vue'
import PageState from '@/components/common/PageState.vue'
import SearchPanel from '@/components/common/SearchPanel.vue'
import StatusTag from '@/components/common/StatusTag.vue'
import { getRegistrationService } from '@/services/registration'
import { isServiceError, type UiError } from '@/types/api'
import type { Registration, RegistrationQuery, RegistrationStatus } from '@/domains/registration'
import type { TableColumn } from '@/types/ui'
const router = useRouter()
const filters = reactive({
  employeeKeyword: '',
  keyword: '',
  status: '' as RegistrationStatus | '',
  page: 1,
  pageSize: 20,
})
const rows = ref<Registration[]>([])
const total = ref(0)
const loading = ref(false)
const hasLoaded = ref(false)
const error = ref<UiError | null>(null)
const selected = ref<Registration | null>(null)
const action = ref<'signin' | 'absent' | 'complete' | null>(null)
const saving = ref(false)
const columns: TableColumn[] = [
  { key: 'employeeName', label: '员工', width: 130 },
  { key: 'departmentName', label: '部门', width: 140 },
  { key: 'courseName', label: '课程名称', minWidth: 230 },
  { key: 'registeredAtLabel', label: '报名时间', width: 170 },
  { key: 'statusLabel', label: '状态', width: 120, align: 'center' },
]
const tableRows = computed(() =>
  rows.value.map((item) => ({
    ...item,
    registeredAtLabel: format(item.registeredAt),
    statusLabel: label(item.status),
  })),
)
const state = computed(() =>
  loading.value && !hasLoaded.value
    ? 'loading'
    : !loading.value && rows.value.length === 0
      ? filters.employeeKeyword || filters.keyword || filters.status
        ? 'no-result'
        : 'empty'
      : 'default',
)
const dialogVisible = computed({
  get: () => action.value !== null,
  set: (value: boolean) => {
    if (!value && !saving.value) action.value = null
  },
})
const dialogTitle = computed(
  () =>
    ({ signin: '确认签到', absent: '标记缺席', complete: '确认完成培训' })[
      action.value || 'signin'
    ],
)
function format(value: string | null) {
  if (!value) return '—'
  const date = new Date(value)
  return Number.isNaN(date.getTime())
    ? '—'
    : new Intl.DateTimeFormat('zh-CN', { dateStyle: 'short', timeStyle: 'short' }).format(date)
}
function label(status: RegistrationStatus) {
  return (
    {
      REGISTERED: '已报名',
      SIGNED_IN: '已签到',
      ABSENT: '缺席',
      COMPLETED: '已完成',
      CANCELED: '已取消',
      UNKNOWN: '未知状态',
    }[status] ?? '未知状态'
  )
}
function semantic(status: unknown): 'success' | 'warning' | 'error' | 'info' | 'neutral' {
  return ({
    REGISTERED: 'info',
    SIGNED_IN: 'warning',
    ABSENT: 'error',
    COMPLETED: 'success',
    CANCELED: 'neutral',
    UNKNOWN: 'neutral',
  }[String(status) as RegistrationStatus] ?? 'neutral') as
    'success' | 'warning' | 'error' | 'info' | 'neutral'
}
function query(): RegistrationQuery {
  return {
    employeeKeyword: filters.employeeKeyword || undefined,
    keyword: filters.keyword || undefined,
    status: filters.status || undefined,
    page: filters.page,
    pageSize: filters.pageSize,
    sortBy: 'regDate',
    sortDirection: 'desc',
  }
}
async function load() {
  loading.value = true
  error.value = null
  try {
    const result = await (await getRegistrationService()).listManage(query())
    rows.value = result.items
    total.value = result.total
    hasLoaded.value = true
  } catch (caught) {
    if (isServiceError(caught) && caught.ui.kind === 'canceled') return
    rows.value = []
    total.value = 0
    hasLoaded.value = true
    error.value = isServiceError(caught)
      ? caught.ui
      : {
          kind: 'unknown',
          code: 'UNKNOWN',
          message: '签到列表加载失败。',
          fieldErrors: [],
          retryable: false,
          resultUnknown: false,
        }
  } finally {
    loading.value = false
  }
}
async function sync(next: Partial<typeof filters>, reset = false) {
  Object.assign(filters, next)
  if (reset) filters.page = 1
  await load()
}
function open(row: Registration, next: 'signin' | 'absent' | 'complete') {
  selected.value = row
  action.value = next
}
async function submit() {
  if (!selected.value || !action.value) return
  saving.value = true
  try {
    const service = await getRegistrationService()
    if (action.value === 'signin') await service.signIn(selected.value.id)
    else if (action.value === 'absent') await service.markAbsent(selected.value.id)
    else await service.complete(selected.value.id)
    action.value = null
    await load()
  } catch (caught) {
    if (isServiceError(caught) && caught.ui.resultUnknown) {
      action.value = null
      await load()
    } else if (isServiceError(caught)) error.value = caught.ui
  } finally {
    saving.value = false
  }
}
onMounted(load)
</script>
<template>
  <section class="business-list">
    <PageHeader title="报名与签到" description="维护培训报名、签到和完成状态。" /><SearchPanel
      :expanded="true"
      :searching="loading"
      @search="sync({}, true)"
      @reset="sync({ employeeKeyword: '', keyword: '', status: '', page: 1, pageSize: 20 }, true)"
      ><el-input
        v-model="filters.employeeKeyword"
        clearable
        aria-label="员工"
        placeholder="员工姓名或工号" /><el-input
        v-model="filters.keyword"
        clearable
        aria-label="课程"
        placeholder="课程名称" /><el-select
        v-model="filters.status"
        clearable
        aria-label="报名状态"
        placeholder="报名状态"
        ><el-option label="已报名" value="REGISTERED" /><el-option
          label="已签到"
          value="SIGNED_IN" /><el-option label="缺席" value="ABSENT" /><el-option
          label="已完成"
          value="COMPLETED" /></el-select></SearchPanel
    ><PageState v-if="state === 'loading'" state="loading" /><PageState
      v-else-if="error?.kind === 'forbidden'"
      state="forbidden"
      :description="error.message"
    /><PageState
      v-else-if="error"
      state="error"
      :description="error.message"
      primary-label="重新加载"
      @primary="load"
    /><PageState
      v-else-if="state === 'empty'"
      state="empty"
      title="暂无报名记录"
      description="课程报名后，记录会显示在这里。"
      compact
    /><PageState
      v-else-if="state === 'no-result'"
      state="no-result"
      secondary-label="清空筛选"
      compact
      @secondary="sync({ employeeKeyword: '', keyword: '', status: '', page: 1 }, true)"
    /><DataTable v-else :columns="columns" :rows="tableRows" has-actions
      ><template #cell="{ column, row }"
        ><StatusTag
          v-if="column.key === 'statusLabel'"
          :label="String(row.statusLabel)"
          :semantic="semantic(row.status)"
        /><span v-else>{{ row[column.key] || '—' }}</span></template
      ><template #actions="{ row }"
        ><AppButton
          label="查看"
          variant="text"
          @click="router.push({ name: 'registration-detail', params: { id: row.id } })" /><AppButton
          v-if="row.status === 'REGISTERED'"
          label="签到"
          variant="text"
          @click="open(row as unknown as Registration, 'signin')" /><AppButton
          v-if="row.status === 'REGISTERED'"
          label="缺席"
          variant="text"
          @click="open(row as unknown as Registration, 'absent')" /><AppButton
          v-if="row.status === 'SIGNED_IN'"
          label="完成"
          variant="text"
          @click="open(row as unknown as Registration, 'complete')" /></template></DataTable
    ><AppPagination
      v-if="state === 'default' && total"
      :page="filters.page"
      :page-size="filters.pageSize"
      :total="total"
      :disabled="loading"
      @update:page="sync({ page: $event })"
      @update:page-size="sync({ pageSize: $event }, true)"
    /><ConfirmDialog
      v-model="dialogVisible"
      :title="dialogTitle"
      :description="
        action === 'signin'
          ? '确认将该报名记录更新为已签到。'
          : action === 'absent'
            ? '确认标记该员工缺席本次培训。'
            : '确认该员工已完成培训并进入证书候选。'
      "
      :confirm-label="action === 'absent' ? '确认缺席' : '确认'"
      :type="action === 'absent' ? 'danger' : 'confirm'"
      :loading="saving"
      @confirm="submit"
    />
  </section>
</template>
<style scoped>
.business-list {
  display: grid;
  gap: var(--space-6);
}
</style>
