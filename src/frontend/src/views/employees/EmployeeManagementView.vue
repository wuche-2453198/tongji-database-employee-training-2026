...
const columns: TableColumn[] = [
  ...
  { key: 'empName', label: '姓名', width: 110 },
  { key: 'deptName', label: '部门', width: 140 },
  { key: 'position', label: '职位', width: 140 },
...
function validate() {
  ...
  if (dialog.value === 'create' && (!form.loginName.trim() || !form.password))
    return '新增员工时必须填写登录名和初始密码。'
...
async function save() {
  ...
  try {
    ...
    if (dialog.value === 'create')
      ...
      await service.updateEmployee(String(selected.value.empId), {
        empName: form.empName.trim(),
        deptName: form.deptName.trim(),
        position: form.position,
        email: form.email,
    ...
    formError.value = isServiceError(caught) ? caught.ui.message : '保存员工信息失败。'
  } finally {
    saving.value = false
  }
}
async function remove() {
  ...
  saving.value = true
  try {
    await (await getEmployeeService()).deleteEmployee(String(deleting.value.empId))
    deleting.value = null
    await load()
  } catch (caught) {
...
<template>
  <section class="employee-management">
    <PageHeader title="员工管理" description="维护员工基础资料、账号状态与组织归属。"
      ><template #action
        ><AppButton label="新增员工" variant="primary" @click="openCreate" /></template
    ></PageHeader>
    <SearchPanel
      :expanded="true"
      :searching="loading"
      @search="sync({}, true)"
      @reset="sync({ keyword: '', deptName: '', status: '', page: 1 }, true)"
    >
      <el-input
      ...
      <el-select v-model="filters.status" clearable placeholder="员工状态" aria-label="员工状态"
        ><el-option label="在职" value="ACTIVE" /><el-option label="离职" value="RESIGNED"
      /></el-select>
    </SearchPanel>
      ...
      <DataTable :columns="columns" :rows="records" has-actions
        ><template #cell="{ column, row }"
          ><StatusTag
            ...
            :label="row.status === 'ACTIVE' ? '在职' : '离职'"
            :semantic="row.status === 'ACTIVE' ? 'success' : 'neutral'"
          /><span v-else>{{ row[column.key] || '—' }}</span></template
        ><template #actions="{ row }"
          ><AppButton
            label="编辑"
            variant="text"
            @click="openEdit(row as EmployeeSummary)" /><AppButton
            v-if="row.status === 'ACTIVE'"
            label="停用"
            variant="text"
            @click="deleting = row as EmployeeSummary" /></template
      ></DataTable>
      <AppPagination
        v-if="total"
        :page="filters.page"
    ...
    <el-dialog
      ...
      ><el-alert
        ...
        type="error"
        :closable="false"
        show-icon /><el-form label-position="top"
        ><FormField v-if="dialog === 'create'" label="登录名" required
          ><el-input v-model="form.loginName" /></FormField
        ><FormField label="员工姓名" required><el-input v-model="form.empName" /></FormField
        ><FormField label="所属部门" required><el-input v-model="form.deptName" /></FormField>
        <div class="grid">
          <FormField label="职位"><el-input v-model="form.position" /></FormField
          ><FormField label="入职日期" :required="dialog === 'create'"
            ><el-date-picker
        ...
        <div class="grid">
          ...
          ><FormField label="电话"><el-input v-model="form.phone" /></FormField>
        </div>
        <FormField v-if="dialog === 'create'" label="初始密码" required
          ><el-input v-model="form.password" type="password" show-password /></FormField
        ><FormField v-else label="账号状态"
          ><el-select v-model="form.status" style="width: 100%"
            ><el-option label="在职" value="ACTIVE" /><el-option
              label="离职"
              value="RESIGNED" /></el-select></FormField></el-form
      ><template #footer
        ><AppButton label="取消" :disabled="saving" @click="dialog = null" /><AppButton
          label="保存"
          variant="primary"
          :loading="saving"
          @click="save" /></template
    ...
    <ConfirmDialog
      ...
      title="停用员工"
      :description="`确定将 ${deleting?.empName ?? ''} 标记为离职吗？该操作会阻止其继续使用系统。`"
      confirm-label="确认停用"
      type="danger"
      :loading="saving"
