...
export const mockDepartmentBudgetService: DepartmentBudgetService = {
  ...
  async create(input, options) {
    await mockWait(options?.signal)
    if (records.some((record) => record.departmentName === input.departmentName.trim()))
    ...
    const record = {
      ...
      usedBudget: 0,
      remainingBudget: input.annualBudget,
    }
  ...
  async update(id, input, options) {
    ...
    const record = {
      ...
      departmentName: input.departmentName.trim(),
      annualBudget: input.annualBudget,
      remainingBudget: input.annualBudget - existing.usedBudget,
    }
