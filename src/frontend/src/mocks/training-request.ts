...
function canReadRequest(request: TrainingRequest): boolean {
  ...
  return actor.role === 'DEPT_MANAGER' && request.departmentName === actor.departmentName
}
...
async function transitionRequest(
  ...
  if (action !== 'file' && actor.role !== 'DEPT_MANAGER') throw domainError('REQUEST_FORBIDDEN')
...
export const mockTrainingRequestService: TrainingRequestService = {
  ...
  async listDepartment(query, options) {
    await mockWait(options?.signal, 50)
    if (getMockActor().role !== 'DEPT_MANAGER') throw domainError('REQUEST_FORBIDDEN')
    throwScenarioForQuery()
