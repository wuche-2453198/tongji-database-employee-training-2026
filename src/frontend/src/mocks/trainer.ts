...
export const mockTrainerService: TrainerService = {
  async list(query, options) {
    ...
    const items = records.filter(
      (record) =>
        (!name || record.name.toLowerCase().includes(name)) &&
        (!company || record.company?.toLowerCase().includes(company)) &&
        (!query.internal || (record.isInternal ? 'Y' : 'N') === query.internal),
    )
  ...
  async create(input, options) {
    ...
    const record: Trainer = {
      id: String(Math.max(...records.map((item) => Number(item.id)), 3000) + 1),
  ...
  async update(id, input, options) {
    ...
    if (!existing) throw createMockError('not-found')
    const record = {
      ...existing,
      name: input.name.trim(),
      title: input.title?.trim() || null,
