...
export function createMockCourseService(
  ...
  return {
    ...
    async createCourse(input, options) {
      ...
      const id = String(
        Math.max(
          ...mockBusinessRepository.getState().courses.map((item) => Number(item.id)),
          4000,
        ) + 1,
      )
      const course: CourseDetail = {
        ...
        trainerName: `讲师 ${input.trainerId}`,
        ...
        registeredCount: 0,
        remainingSeats: input.maxStudents,
        description: '暂无课程介绍。',
        objectives: [],
        hours: input.durationHours,
        organizer: `部门 ${input.deptId}`,
        trainer: {
          id: input.trainerId,
          name: `讲师 ${input.trainerId}`,
          title: '—',
          department: '—',
          expertise: '—',
        },
        materials: [],
        eligibility: {
          apply: { allowed: false, reason: '课程尚未发布。' },
          register: { allowed: false, reason: '课程尚未发布。' },
        },
        trainerId: input.trainerId,
        deptId: input.deptId,
        budgetAmount: input.budgetAmount,
        preTestUrl: input.preTestUrl || null,
        postTestUrl: input.postTestUrl || null,
    ...
    async updateCourse(courseId, input, options) {
      ...
      mockBusinessRepository.update((next) => {
        ...
        Object.assign(course, {
          ...
          location: input.location,
          maxStudents: input.maxStudents,
          remainingSeats: Math.max(0, input.maxStudents - (course.registeredCount ?? 0)),
          hours: input.durationHours,
          organizer: `部门 ${input.deptId}`,
          trainerId: input.trainerId,
          deptId: input.deptId,
