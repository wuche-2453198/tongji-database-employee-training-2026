...
describe('M4 Mock 认证与权限', () => {
  ...
  const accounts: Array<{ account: string; role: AppRole; name: string }> = [
    ...
    { account: 'manager.demo', role: 'DEPT_MANAGER', name: '李主管' },
  ...
  it('多角色占位按权限并集生成菜单和操作权限', () => {
    const roles: AppRole[] = ['EMPLOYEE', 'DEPT_MANAGER']
    const user: AppUser = {
    ...
    expect(hasPermission(user, 'request.file')).toBe(false)
  })

  it('详情页按当前角色保持对应业务菜单选中态', () => {
    expect(getActiveMenuKey('request-detail', ['EMPLOYEE'])).toBe('my-requests')
    expect(getActiveMenuKey('request-detail', ['DEPT_MANAGER'])).toBe('department-approval')
