...
当前状态：后端认证、讲师与课程、培训申请审批、成果评估四个模块已合入 `develop`，可通过 Swagger 查看；组织基础、报名签到和前端工程在独立分支开发，尚未合入。技术、数据、状态、权限和核心业务决策已通过；接口路径、DTO 和页面映射为推荐基线，最终以运行后端后的 Swagger/OpenAPI 为准。
...
| 模块 | 接口前缀 | 说明 |
| --- | --- | --- |
| 组织基础 | `/api/employees`、部门预算、`/api/blacklist` | 员工、部门培训预算、黑名单 |
