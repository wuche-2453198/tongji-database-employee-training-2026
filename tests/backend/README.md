...
| 文件 | 项数 | 覆盖内容 |
| --- | --- | --- |
| `CourseServiceTests.cs` | 22 | 课程创建默认 `DRAFT`、部门存在性、发布后冻结字段、状态机与并发、容量统计、发布必填项、预算占用事务 |
| `CourseControllerTests.cs` | 5 | 员工课程列表强制 `PUBLISHED`、HR 可读草稿、员工读写越权边界 |
| `TrainerServiceTests.cs` | 9 | 讲师 CRUD、过滤、分页、星级 1~5、内部讲师 `Y/N`、缺失记录 |
| `ResultModuleTests.cs` | 18 | 证书状态映射（`EXPIRE_DATE` 为空/过期/临期）、证书资格（`COMPLETED`、`POST >= 60`）、重复发证、越权查询；PRE 需有效报名、POST 需完成培训、重复成绩 409、分数/类型/时间校验、课程 404、提升率 0 分处理；评分资格与重复 |
| `RequestValidationTests.cs` | 7 | `Trainer`/`Course` DTO 的电话长度、学时与人数精度边界，对照 `V001` |
| `RepositoryContractTests.cs` | 6 | Dapper 命令绑定、分页参数、条件 `UPDATE`、数字/空值映射、输出 ID、未配置数据库快速失败 |
...
| `BlacklistServiceTests.cs` | 15 | 黑名单创建权限与部门范围、重复冲突、结束日期晚于今天、结束日期不早于开始日期、解除与列表范围 |
| `HttpAuthorizationTests.cs` | 1（仅 `--auth-http`） | 四角色登录、JWT claim、`/api/auth/me`，以及 11 个课程/讲师/成绩路由 × 6 种鉴权组合共 66 次请求 |
| 合计 | **115**（加 `--auth-http` 为 **116**） | |

## 最近验证记录（2026-09-11）

- `dotnet run --project tests/backend/TrainingManagement.Api.ModuleTests.csproj`：**115/115 通过**。
- `dotnet run --project tests/backend/TrainingManagement.Api.ModuleTests.csproj -- --auth-http`：**116/116 通过**，含 66 次 HTTP 权限请求。
- 成绩模块：PRE 改为按有效报名（`REGISTERED`/`SIGNED_IN`/`COMPLETED`）准入并校验课程未开始；POST 需报名 `COMPLETED`；同一员工+课程+类型重复录入返回 409。
- 证书模块：`EXPIRE_DATE` 为空按长期有效（`VALID`）处理，修复前端“未知状态”。
...
- `dotnet run --project tests/backend/TrainingManagement.Api.ModuleTests.csproj`：**84/84 通过**（当时清单）。
...
课程/讲师/成绩共 11 个路由分别检查无 token、无效 token 和四种角色，共 66 次权限请求：

- 无 token / 无效 token：401。
- 四种角色查询课程：通过鉴权。
- EMPLOYEE / DEPT_MANAGER 访问课程写接口、讲师管理或成绩录入：403。
...
- 组织与报名模块已合入 `develop`（本轮测试数 116），仍需补跨模块用例：报名→完成→评分/测试→证书的真库链路。
